...
В 1.1.7 главный экран оформлен как семейный альбом: состояние соединения
над домом в арочной рамке, приветствие и московское время в отдельной строке
под иллюстрацией. Кнопка остаётся на двери, без текстовой подписи. Утро,
день, вечер и ночь сохраняют состояния включённого/выключенного VPN и плавное
изменение освещения без смены неба. Во время подключения показывается
«Подключаемся», а не преждевременное подтверждение защиты. На небольшом экране
доступна прокрутка; обновление APK по-прежнему отображается над главным экраном.
